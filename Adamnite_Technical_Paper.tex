...
This setup is very similar to that proposed in EIP-3102, and it provides a deterministic mapping between addresses and other state values. The actual values for code and data are stored in a separate database, presumably one that also has direct access to the on-chain trie, and in some implementations, may even store a copy, thereby also storing the entirety of the state.
\subsection{Virtual Machine Instruction Set}

A majority of the ADVM's numeric and operation instruction is directly derived from WASM. This allows for computation that is native to most computers. WASM is natively little-endian, and thus by extent, a majority of the ADVM's operations are also little-endian. Unique operations (such as Elliptic Curve Cryptography or generalized crypto functions) can be implemented as WASM modules. A set of WASM instructions can be found \href{https://webassembly.github.io/spec/core/binary/instructions.html}{here}.\\

However, the ADVM does implement specific instructions that are specific to its environment. These instructions, along with a description of their operations, can be found below. These instructions are mostly adopted from the Ethereum Virtual Machine. These instructions are implemented in a reserve WASM Address.\\

\begin{tabular}{ |p{3cm}|p{3cm}|p{3cm}|  }
\hline
\multicolumn{3}{|c|}{Environmental Instruction Set (0xF} \\
\hline
 Name & Short Description & Value \\
\hline
ADDRESS & The Address of the contract & 0xF0 \\
BALANCE & Balance of the contract. & 0xF1\\
CALLER & The account that called the contract. & 0xF2\\
DATASIZE & The size of the data being passed. & 0xF3\\
CODESIZE & The size of the code being executed. & 0xF4\\
ATETOTAL & The total amount of ATE allocated to this call, & 0xF5\\
COPYDATA & Copy the current data input to memory. & 0xF6\\
COPYCODE & Copy the current code input to memory. & 0xF7\\
GETCODE & Return a copy of the current code to read. & 0xF8\\
VALUE & Return the amount of Micalli, if any, in the message. & 0xF9\\
TIMESTAMP & The current block's timestamp. & 0xFA\\
CALLERBALANCE & The balance of the caller. & 0xFB\\
\hline
\end{tabular}
